    ...
    <div className="bg-[#FFFDF7]">
      ...
      <section className="relative pt-32 pb-24 md:pt-40 md:pb-32 px-6 md:px-12 overflow-hidden" data-testid="tech-hero">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          <motion.div {...heroAnim}>
            ...
            <p className="font-body text-base md:text-lg text-[#6B7280] max-w-2xl mx-auto leading-relaxed">
      ...
      {sections.map((section, i) => (
        <section key={section.title}
          className={`py-24 md:py-32 px-6 md:px-12 ${i % 2 === 0 ? 'bg-[#FFFDF7]' : 'bg-white'}`}
          ...
          <div className="max-w-5xl mx-auto">
            <motion.div {...fadeUp} className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              <div className="lg:col-span-4">
                ...
                <h2 className="font-heading text-2xl md:text-3xl font-semibold uppercase tracking-normal text-[#1A1A2E] mb-2">
                  {section.title}
                </h2>
                <p className="font-heading text-sm uppercase tracking-wider text-[#007AFF]">{section.subtitle}</p>
              </div>
              <div className="lg:col-span-8">
                <p className="font-body text-base text-[#6B7280] leading-relaxed mb-6">{section.content}</p>
                <div className="bg-[#007AFF]/10 border-l-2 border-[#007AFF] pl-6 py-3">
                  <p className="font-body text-sm text-[#1A1A2E] font-medium">{section.highlight}</p>
                </div>
              </div>
            </motion.div>
          </div>
