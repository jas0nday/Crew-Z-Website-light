...
export default function SupportPage() {
  ...
  return (
    <div className="bg-[#0A0A0A]">
      ...
      {/* FAQ */}
      <section className="py-24 md:py-32 px-6 md:px-12 bg-[#111111]" data-testid="support-faq">
        <div className="max-w-3xl mx-auto">
          <motion.div {...fadeUp} className="text-center mb-16">
            <HelpCircle className="w-10 h-10 text-[#007AFF] mx-auto mb-4" />
            <h2 className="font-heading text-4xl md:text-5xl font-bold uppercase tracking-tight text-white">Frequently Asked Questions</h2>
          </motion.div>
          <motion.div {...fadeUp}>
            <Accordion type="single" collapsible className="space-y-2" data-testid="support-faq-accordion">
              {generalFaqs.map((faq, i) => (
                <AccordionItem key={i} value={`faq-${i}`} className="bg-[#0A0A0A] border border-white/10 rounded-sm px-6">
                  <AccordionTrigger className="font-body text-sm text-white hover:text-[#007AFF] py-4" data-testid={`support-faq-${i}`}>
                    {faq.q}
                  </AccordionTrigger>
      ...
      <section className="py-24 md:py-32 px-6 md:px-12" data-testid="contact-section">
        <div className="max-w-2xl mx-auto">
          <motion.div {...fadeUp} className="text-center mb-16">
            ...
            <h2 className="font-heading text-4xl md:text-5xl font-bold uppercase tracking-tight text-white">Get in Touch</h2>
            <p className="font-body text-base text-[#A1A1AA] mt-4">Can't find the answer? Send us a message.</p>
          </motion.div>

          {sent ? (
            <motion.div {...fadeUp} className="bg-[#111111] border border-[#007AFF]/30 rounded-sm p-8 text-center" data-testid="contact-success">
              <CheckCircle className="w-12 h-12 text-[#007AFF] mx-auto mb-4" />
              <h3 className="font-heading text-xl text-white uppercase mb-2">Message Sent</h3>
              <p className="font-body text-sm text-[#A1A1AA]">We'll get back to you as soon as possible.</p>
            </motion.div>
          ) : (
            <motion.form {...fadeUp} onSubmit={handleSubmit} className="space-y-4" data-testid="contact-form">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block font-body text-xs text-[#A1A1AA] uppercase tracking-wider mb-1">Name *</label>
                  <Input value={form.name} onChange={update('name')} required
                    className="bg-[#111111] border-white/10 text-white" data-testid="contact-name" />
                </div>
                <div>
                  <label className="block font-body text-xs text-[#A1A1AA] uppercase tracking-wider mb-1">Email *</label>
                  <Input type="email" value={form.email} onChange={update('email')} required
                    className="bg-[#111111] border-white/10 text-white" data-testid="contact-email" />
                </div>
              </div>
              <div>
                <label className="block font-body text-xs text-[#A1A1AA] uppercase tracking-wider mb-1">Subject *</label>
                <Input value={form.subject} onChange={update('subject')} required
                  className="bg-[#111111] border-white/10 text-white" data-testid="contact-subject" />
              </div>
              <div>
                <label className="block font-body text-xs text-[#A1A1AA] uppercase tracking-wider mb-1">Message *</label>
                <Textarea value={form.message} onChange={update('message')} required rows={5}
                  className="bg-[#111111] border-white/10 text-white resize-none" data-testid="contact-message" />
              </div>
