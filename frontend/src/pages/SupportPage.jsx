          ...
          <Accordion type="single" collapsible className="space-y-2" data-testid="support-faq-accordion">
            {generalFaqs.map((faq) => (
              <AccordionItem key={faq.q} value={`faq-${faq.q.slice(0, 20)}`} className="bg-[#FFFDF7] border border-gray-200 rounded-xl px-6">
                <AccordionTrigger className="font-body text-sm text-[#1A1A2E] hover:text-[#007AFF] py-4" data-testid={`support-faq-${faq.q.slice(0, 15)}`}>{faq.q}</AccordionTrigger>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {sent ? (
          ...
          <motion.form {...fadeUp} onSubmit={handleSubmit} className="space-y-4" data-testid="contact-form">
            {error && <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4"><p className="font-body text-sm text-red-500" data-testid="contact-error">{error}</p></div>}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><label className="block font-body text-xs text-[#6B7280] uppercase tracking-wider mb-1">Name *</label><Input value={form.name} onChange={update('name')} required className="bg-white border-gray-300 text-[#1A1A2E]" data-testid="contact-name" /></div>
              <div><label className="block font-body text-xs text-[#6B7280] uppercase tracking-wider mb-1">Email *</label><Input type="email" value={form.email} onChange={update('email')} required className="bg-white border-gray-300 text-[#1A1A2E]" data-testid="contact-email" /></div>
            </div>
            <div><label className="block font-body text-xs text-[#6B7280] uppercase tracking-wider mb-1">Subject *</label><Input value={form.subject} onChange={update('subject')} required className="bg-white border-gray-300 text-[#1A1A2E]" data-testid="contact-subject" /></div>
            <div><label className="block font-body text-xs text-[#6B7280] uppercase tracking-wider mb-1">Message *</label><Textarea value={form.message} onChange={update('message')} required rows={5} className="bg-white border-gray-300 text-[#1A1A2E] resize-none" data-testid="contact-message" /></div>
            <button type="submit" disabled={loading} className="w-full bg-[#007AFF] text-white font-heading uppercase tracking-widest text-sm py-4 rounded-full hover:bg-[#3395FF] hover:shadow-lg hover:shadow-blue-100 transition-all disabled:opacity-50 flex items-center justify-center gap-2" data-testid="contact-submit-btn">
